...
BufferGroup& IoUringEngine::register_buffer_group(unsigned gid, size_t entries, size_t buf_size) {
    auto it = m_buffer_groups.find(gid);
    if (it != m_buffer_groups.end()) return *it->second;
...
void* IoUringEngine::get_buffer(unsigned gid, unsigned bid) noexcept {
    auto it = m_buffer_groups.find(gid);
    if (it != m_buffer_groups.end()) return it->second->get_buffer(bid);
    return nullptr;
}
